...
Let's say we're playing a game where we want to assign their coins and their outcomes to players A and B and the player with the most heads will win. We could do this by going through each coin and assigning it to A or B using the bernoulli distribution where p = .5, meaning there's an equal chance it will be assigned to A or B. But then we'd have to do this process a 100 times. Instead, we could *group* the coins by their five possible outcomes, recording how many coins had 1 flip that was heads, how many 2 flips that were both tails, etc. Now we'll have a 5x3 table - the columns being the number of heads, the number of flips, and the number of coins with that combination. 

We can now use the *binomial* distribution to assign coins to A or B. We keep p = .5 and n is the number of coins with that combination. We'll then get how many of those coins with that combination should go to A. Because the bernoulli distribution was just a special case of the binomial distribution where n = 1, we're doing the *same* process as before, but we only need to do 5 computations! 
